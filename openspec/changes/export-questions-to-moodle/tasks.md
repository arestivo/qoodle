...
- [x] Add `grading_mode` field to `Exam` model in `apps/exams/models.py`
...
- [x] Add `default_grade` field to `QuestionPool` model in `apps/exams/models.py`
  - DecimalField with `max_digits=5`, `decimal_places=2`
  - Default: `Decimal('1.0')`
  - Validators: `[MinValueValidator(Decimal('0.1'))]`
  - Help text: "Point value for questions in this pool"

- [x] Add `get_grading_mode_display_verbose()` method to `Exam` model
  - Returns detailed description: "Single Choice (100% / -33% fractions)" or "Multiple Choice (70/10 to 75/5 fractions)"

### Migration

- [x] Run `poetry run python manage.py makemigrations exams` to create migration
- [x] Review generated migration file for correctness
- [x] Run `poetry run python manage.py migrate` to apply migration
- [x] Verify migration with `poetry run python manage.py showmigrations exams`

### Verification

- [x] Run `poetry run python manage.py shell` and test:
  ```python
...
- [x] Create new file `apps/exams/moodle_export.py` (empty file to start)

- [x] Implement `calculate_fractions()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `format_html_for_moodle()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `extract_language_text()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `generate_variable_value()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `substitute_markers()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `generate_variant()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `get_choices_for_variant()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `generate_all_variants()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Implement `generate_moodle_xml()` function in `apps/exams/moodle_export.py`
  ```python
...
- [x] Create test template with variables in Django shell:
  ```python
...
- [x] Test variant generation in shell:
  ```python
...
- [x] Modify `ExamForm` in `apps/exams/forms.py` to include `grading_mode` field
  - Add `'grading_mode'` to `fields` list
  - Widget: `forms.RadioSelect()`

- [x] Create `PoolGradeForm` in `apps/exams/forms.py`
...
- [x] Create `ExamExportView` in `apps/exams/views.py`
  ```python
...
- [x] Create `PoolUpdateGradeView` in `apps/exams/views.py`
  ```python
...
- [x] Add export route to `apps/exams/urls.py`:
  ```python
  path('<uuid:pk>/export/', ExamExportView.as_view(), name='export')
  ```

- [x] Add pool grade update route to `apps/exams/urls.py`:
  ```python
...
- [x] Modify `apps/exams/templates/exams/exam_form.html` to add grading mode section
...
- [x] Modify `apps/exams/templates/exams/exam_detail.html` to add export form
...
- [x] Modify `apps/exams/templates/exams/exam_detail.html` to show grading mode badge
...
- [x] Modify `apps/exams/templates/exams/exam_detail.html` to show pool grades
...
- [x] Modify `apps/exams/templates/exams/exam_detail.html` to add pool grade edit modal
...
- [x] Run development server: `poetry run python manage.py runserver`

- [x] Test grading mode UI:
...
- [x] Create test class `CalculateFractionsTests` in `apps/exams/tests.py`
...
- [x] Create test class `VariantGenerationTests` in `apps/exams/tests.py`
...
- [x] Create test class `UniquenessValidationTests` in `apps/exams/tests.py`
...
- [x] Create test class `MoodleXMLGenerationTests` in `apps/exams/tests.py`
...
- [x] Create test class `LanguageExtractionTests` in `apps/exams/tests.py`
  - Test extraction of specified language
  - Test fallback to first available language
  - Test fallback when target language missing

- [x] Create test class `MarkdownConversionTests` in `apps/exams/tests.py`
...
- [x] Create test class `ExamExportViewTests` in `apps/exams/tests.py`
...
- [x] Create test class `PoolGradeUpdateTests` in `apps/exams/tests.py`
...
- [x] Create test class `GradingModeFormTests` in `apps/exams/tests.py`
...
- [x] Create test exam in Django admin with:
  - 3 question pools
...
- [x] Export exam and validate XML:
...
- [x] Test Moodle import (if test instance available):
...
- [x] Run test suite with coverage: `poetry run pytest --cov=apps/exams --cov-report=term-missing`
- [x] Verify coverage is >80% for `apps/exams/moodle_export.py`
- [x] Verify coverage is >80% for export-related views
- [x] Add additional tests for any uncovered lines

## Phase 5: Error Handling and Polish (Day 9)

### Error Handling

- [x] Add error handling for empty exams in `ExamExportView`
  - Check if exam has no pools
  - Show error message: "Cannot export exam with no question pools"
  - Redirect to exam detail

- [x] Add error handling for pools with no templates
  - Check if any pool has zero templates
  - Show error message: "Pool {order} has no templates"
  - Redirect to exam detail

- [x] Add error handling for templates with no choices
  - Check if template has zero choices
  - Show error message: "Template '{title}' has no answer choices"
  - Redirect to exam detail

- [x] Add user-friendly error messages for variant generation failures
...
- [x] Add user-friendly error for >6 choices in multi mode
...
- [x] Add docstrings to all functions in `apps/exams/moodle_export.py`
  - Include type hints
  - Document parameters and return values
  - Include usage examples for key functions

- [x] Add help text to grading mode field explaining fraction schemes
